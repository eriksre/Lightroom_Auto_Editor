...
def masked_mse_loss(
    ...
    """
    Compute MSE loss using only valid (masked-in) target values.
    """
    squared_error = ((predictions - targets) ** 2) * masks
    # Average over valid targets per sample, then average over samples
    valid_counts = masks.sum(dim=1).clamp(min=1)
    per_sample_loss = squared_error.sum(dim=1) / valid_counts
...
def train_epoch(
    ...
    """Train for one epoch with masked MSE loss."""
    model.train()
    total_loss = 0.0
    total_samples = 0
...
def evaluate(
    ...
    """Evaluate the model with masked MSE loss."""
    model.eval()
    total_loss = 0.0
    total_samples = 0
