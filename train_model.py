...
def train_epoch(
    ...
    """Train for one epoch with masked Mahalanobis loss."""
    model.train()
    total_loss = 0.0
    total_samples = 0
    ...
    for features, targets, masks in dataloader:
        ...
        loss = masked_mahalanobis_loss(predictions, targets, masks, precision)
        loss.backward()
        optimizer.step()
...
def evaluate(
    ...
    """Evaluate the model with masked Mahalanobis loss."""
    model.eval()
    total_loss = 0.0
    total_samples = 0
...
def train(
    ...
    for epoch in range(epochs):
        train_loss = train_epoch(model, train_loader, optimizer, device, target_precision_t)
        val_loss = evaluate(model, val_loader, device, target_precision_t)
