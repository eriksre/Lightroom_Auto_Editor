...
log-likelihood with a *learned low-rank + diagonal global covariance*:
  Σ = diag(σ²) + U Uᵀ
This captures cross-slider correlation with far fewer parameters than a full
...
class LearnedLowRankCovariance(nn.Module):
    """
    Learn a global SPD covariance matrix via:
      Σ = diag(σ²) + U Uᵀ
    where σ² is positive and U has shape (n_targets, rank).
    """
    ...
    def __init__(
        ...
        *,
        rank: int = 8,
        init_diag: np.ndarray | None = None,
        min_var: float = 1e-4,
    ) -> None:
        super().__init__()
        ...
        self.raw_log_diag = nn.Parameter(torch.zeros(self.n_targets, dtype=torch.float32))
        self.factors = nn.Parameter(torch.zeros((self.n_targets, self.rank), dtype=torch.float32))

        if init_diag is not None:
            init_diag = np.asarray(init_diag, dtype=np.float32).reshape(-1)
            if init_diag.shape != (self.n_targets,):
                raise ValueError(f"init_diag shape {init_diag.shape} != ({self.n_targets},)")
            init_diag = np.clip(init_diag, a_min=self.min_var, a_max=None)
            with torch.no_grad():
                # raw_log_diag is passed through softplus, so we need softplus^{-1}.
                # Ensure strictly positive argument to avoid -inf from log(0).
                y = (init_diag - self.min_var).astype(np.float64)
                y = np.clip(y, a_min=1e-6, a_max=None)
                inv_softplus = np.log(np.expm1(y))
                self.raw_log_diag.copy_(torch.from_numpy(inv_softplus.astype(np.float32)))

    def diag_var(self) -> torch.Tensor:
        return torch.nn.functional.softplus(self.raw_log_diag) + self.min_var

    def covariance_matrix(self) -> torch.Tensor:
        d = self.diag_var()
        return torch.diag(d) + (self.factors @ self.factors.T)


def masked_gaussian_nll_with_learned_covariance(
    predictions: torch.Tensor,
    targets: torch.Tensor,
    masks: torch.Tensor,
    ...
    *,
    ...
    For each sample, uses the observed target subset S (from masks) and computes:
      0.5 * ( r_Sᵀ Σ_SS^{-1} r_S + log|Σ_SS| )
    where Σ is a learned global covariance and r = ŷ - y.

    Notes:
    - This respects cross-target correlation via Σ_SS.
    - It remains well-posed because the log-determinant term discourages trivial scaling.
    """
    mask_bool = masks > 0.5
    batch_size, n_targets = mask_bool.shape
    if n_targets != covariance.shape[0] or n_targets != covariance.shape[1]:
        raise ValueError(
            f"covariance shape {tuple(covariance.shape)} does not match n_targets={n_targets}"
        )
    ...
    for start, end in zip(boundaries[:-1], boundaries[1:]):
        ...
        S_sub = covariance.index_select(0, obs).index_select(1, obs)

        S_sub = S_sub + jitter * torch.eye(k, device=predictions.device, dtype=predictions.dtype)
        L = torch.linalg.cholesky(S_sub)  # lower-triangular, S_sub = L Lᵀ

        # Quadratic form q = rᵀ Σ^{-1} r = || L^{-1} rᵀ ||²
        y = torch.linalg.solve_triangular(L, r.T, upper=False)  # (k, n_group)
        q = (y * y).sum(dim=0)

        logdetS = 2.0 * torch.log(torch.diagonal(L)).sum()
        per_sample = 0.5 * (q + logdetS) / float(k)
...
def train_epoch(
    ...
    device: torch.device,
    covariance_module: LearnedLowRankCovariance,
) -> float:
    """Train for one epoch with masked Gaussian NLL (low-rank covariance)."""
    model.train()
    covariance_module.train()
    total_loss = 0.0
    total_samples = 0
    ...
    for features, targets, masks in dataloader:
        ...
        covariance = covariance_module.covariance_matrix()
        loss = masked_gaussian_nll_with_learned_covariance(
            predictions, targets, masks, covariance
        )
        loss.backward()
        optimizer.step()
...
def evaluate(
    ...
    """Evaluate the model with masked Gaussian NLL (low-rank covariance)."""
    model.eval()
    covariance_module.eval()
    total_loss = 0.0
    total_samples = 0
...
def train(
    ...
    dropout: float = 0.2,
    val_split: float = 0.2,
    save_path: str = "model_weights/lightroom_model_lowrank_cov_nll.pt",
    seed: int = 42,
    ...
    # Initialize diagonal variances from the empirical (masked) covariance estimate.
    # We initialize factors at 0 so the model starts near a diagonal covariance and
    # can learn correlations through U Uᵀ.
    target_cov_init = compute_masked_covariance(train_targets_norm, train_masks)
    target_cov_init = 0.5 * (target_cov_init + target_cov_init.T)
    init_diag = np.clip(np.diag(target_cov_init).astype(np.float32), a_min=1e-4, a_max=None)
    covariance_module = LearnedLowRankCovariance(
        len(target_names),
        rank=int(cov_rank),
        init_diag=init_diag,
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": covariance_module.parameters(), "weight_decay": 0.0},
        ],
        lr=learning_rate,
    )
    ...
    for epoch in range(epochs):
        train_loss = train_epoch(model, train_loader, optimizer, device, covariance_module)
        val_loss = evaluate(model, val_loader, device, covariance_module)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'target_cov_init': target_cov_init,
                'cov_rank': int(cov_rank),
                'cov_raw_log_diag': covariance_module.raw_log_diag.detach().cpu().numpy(),
                'cov_factors': covariance_module.factors.detach().cpu().numpy(),
                'cov_min_var': float(covariance_module.min_var),
...
def train_ensemble(
    *,
    ...
    dropout: float = 0.2,
    val_split: float = 0.2,
    save_path_template: str = "model_weights/lightroom_model_lowrank_cov_nll_seed{seed}.pt",
    split_seed: int | None = None,
    cov_rank: int = 8,
) -> pd.DataFrame:
    """
...
if __name__ == "__main__":
    ...
    save_path = "model_weights/lightroom_model_lowrank_cov_nll.pt"

    # Ensemble output template (used when len(seeds)>1)
    save_path_template = "model_weights/lightroom_model_lowrank_cov_nll_seed{seed}.pt"
    # ==========================
