...
This variant uses a robust, correlation-aware loss: a masked Student-t
Mahalanobis objective based on a global precision matrix estimated from the
training targets.
"""
...
def masked_student_t_mahalanobis_loss(
    ...
    """
    Robust, correlation-aware loss using a (masked) multivariate Student-t form.

    Uses the Mahalanobis quadratic form q = rᵀ P r, but applies log1p(q / df)
    to reduce sensitivity to outliers (heavy tails), while still respecting
    cross-target correlation via P (precision matrix).
    """
    residual = (predictions - targets) * masks
    projected = residual @ precision
    q = (projected * residual).sum(dim=1).clamp(min=0)
    d = masks.sum(dim=1).clamp(min=1)

    nu = torch.as_tensor(df, dtype=predictions.dtype, device=predictions.device).clamp(min=1e-6)
    per_sample = 0.5 * (nu + d) * torch.log1p(q / nu)
    per_sample = per_sample / d
...
def train_epoch(
    ...
    """Train for one epoch with masked Student-t Mahalanobis loss."""
    model.train()
    total_loss = 0.0
    total_samples = 0
    ...
    for features, targets, masks in dataloader:
        ...
        loss = masked_student_t_mahalanobis_loss(
            predictions, targets, masks, precision, df=student_t_df
        )
        loss.backward()
        optimizer.step()
...
def evaluate(
    ...
    """Evaluate the model with masked Student-t Mahalanobis loss."""
    model.eval()
    total_loss = 0.0
    total_samples = 0
...
def train(
    ...
    dropout: float = 0.2,
    val_split: float = 0.2,
    save_path: str = "model_weights/lightroom_model.pt",
    seed: int = 42,
    ...
    # Build precision matrix for Mahalanobis loss (on normalized targets)
    target_precision = compute_target_precision(train_targets_norm, train_masks)
    target_precision_t = torch.tensor(target_precision, dtype=torch.float32, device=device)
    ...
    for epoch in range(epochs):
        train_loss = train_epoch(
            model, train_loader, optimizer, device, target_precision_t, student_t_df
        )
        val_loss = evaluate(model, val_loader, device, target_precision_t, student_t_df)
...
def train_ensemble(
    *,
    ...
    dropout: float = 0.2,
    val_split: float = 0.2,
    save_path_template: str = "model_weights/lightroom_model_seed{seed}.pt",
    split_seed: int | None = None,
    student_t_df: float = 4.0,
) -> pd.DataFrame:
    """
...
if __name__ == "__main__":
    ...
    save_path = "model_weights/lightroom_model.pt"

    # Ensemble output template (used when len(seeds)>1)
    save_path_template = "model_weights/lightroom_model_seed{seed}.pt"
    # ==========================
